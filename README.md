...
Many students order food on DoorDash when they're in a hurry or looking for something new to eat. However, the ETA on the app is sometimes unreliable. Our goal is to accurately predict the ETA of an order based on attributes/features like the type of cuisine, the number of on-shift dashers, the time the order was placed, etc. Our dataset consists of 197k observations/orders and 16 features that include those mentioned above. We will do regression on the time it takes for orders to be complete. To these ends, we tried a variety of models ranging from linear regression to deep neural networks. Below are the results of our models, our code and methods, and our thought process.
